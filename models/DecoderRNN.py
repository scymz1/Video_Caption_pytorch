...
class DecoderRNN(nn.Module):
    ...
    Args:
        ...
        rnn_dropout_p (float, optional): dropout probability for the output sequence (default: 0.1)
    ...
    def __init__(self,
        ...
        #print(self.dim_output, dim_word)
    ...
    def forward(self,
        ...
        if mode == 'train':
            ...
            for t in range(self.max_length - 1):
                ...
                #print(decoder_input.shape, context.shape)
                decoder_input = self.input_dropout(decoder_input).unsqueeze(1)
                decoder_output, decoder_hidden = self.rnn(
                    decoder_input, decoder_hidden)
                logprobs = F.log_softmax(
                    self.out(decoder_output.squeeze(1)), dim=1)
            #print(len(seq_logprobs), len(seq_logprobs[0]))
            seq_logprobs = torch.cat(seq_logprobs, 1)
            #print(seq_logprobs.shape)
