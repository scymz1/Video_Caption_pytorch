...
def test(model, crit, dataset, vocab, opt):
    ...
    scorer = COCOScorer()
    input_json=json.load(open(opt["input_json"], encoding='utf-8'))
    input_test_json = json.load(open(opt["input_test_json"], encoding='utf-8'))
    gt_dataframe = pd.concat([json_normalize(input_json['sentences']), json_normalize(input_test_json['sentences'])])
...
def main(opt):
    ...
    if opt["model"] == 'S2VTModel':
        ...
        model = S2VTModel(opt["vocab_size"], opt["max_len"], opt["dim_hidden"], opt["dim_word"], opt['dim_vid'], rnn_cell=opt['rnn_type'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_test_json', type=str, default='data/test-video-annotation/test_videodatainfo.json',
                        help='the json file recording sentences of test dataset')
